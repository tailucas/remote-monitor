...
from ADCPi import ADCTimeoutError
...
from tailucas_pylib import log, APP_NAME, app_config, creds, DEVICE_NAME
        ...
        with exception_handler(
            ...
            while not shutting_down:
                ...
                try:
                    ...
                    log.warn(
                        f"Cannot determine duration from {device_params}, using default of {RELAY_DEFAULT_ACTIVE_TIME_SECONDS}s."
                    )
                log.info(f"Relay event for {device_key} with duration {duration}")
                if device_key not in self._relay_mappings:
                    log.error(
                        f"{device_key} does not match any of {self._relay_mappings.keys()}"
                    )
                    continue
...
def main():
    ...
    log.info(
        f"Using RabbitMQ server at {mq_config_server} with {mq_exchange_type} ({mq_device_topic}) exchange {mq_config_exchange}."
    )
    # control listener
        ...
        while not shutting_down:
            ...
            for i in list(input_to_adc.keys()):
                ...
                try:
                    ...
                    log.warning(
                        "Timeout reading value from {} on pin {}.".format(
                            adc_name, pin
                        ),
                        exc_info=True,
                    )
                ...
                if randint(0, 1000) < SAMPLE_INTERVAL_SECONDS * 1000:
                    log.debug(
                        "Comparing {}.{}={} ({}v) to {} ({}) (tolerance: {})".format(
                            adc_name,
                            pin,
                            normalized_value,
                            sampled_value,
                ...
                if i in input_tamper_values:
                    ...
                    if (
                        ...
                        device_event_distinction = "{} {}".format(
                            device_key, tamper_label
                        )
                ...
                if device_key in device_history:
                    ...
                    if (
                        ...
                        if randint(0, 1000) < SAMPLE_INTERVAL_SECONDS * 1000:
                            log.debug(
                                "Debouncing '{}' (detail: {}) activated {} seconds "
                                "ago.".format(
                                    device_event_distinction,
                                    event_detail,
                ...
                log.info(
                    "'{}' (detail: {}, sampled: {})".format(
                        device_event_distinction, event_detail, normalized_value
                    )
                )
